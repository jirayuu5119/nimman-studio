...
- [x] **Step 2: Install and verify age**

Use an official age release, verify its published checksum, and run `age --version` plus `age-keygen --version`.

- [x] **Step 3: Generate identity and encrypt**

Copy the existing Production identity under F:, verify the copy hashes, and use it to validate the already-created encrypted archive under D: outside the repository. The archive contains a SHA-256 manifest generated by the production backup workflow.

- [x] **Step 4: Restore-verify byte-for-byte**
...
- [x] **Step 5: Gate plaintext removal**
